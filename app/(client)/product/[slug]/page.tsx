			...
			<Container className='flex flex-col md:flex-row gap-10 py-10'>
				{product?.image && (
					<div className='w-full md:w-1/2 h-auto border border-darkBlue/20 shadow-md rounded-md group'>
						...
						width={700} height={700} className='w-full max-h-[500px] object-cover group-hover:scale-110 rounded-md hoverEffect'
						/>
					</div>
				)}
					...
					<div className='flex items-center gap-2'>
						<div className='flex items-center text-gray-500 gap-1'>
							{Array.from({length: 5}).map((_, index)=>{
								...
								return (
									<LuStar fill={!isLastStar ? '#fca99b' : 'transparent'} key={index} className={`${isLastStar ? 'text-gray-500' : 'text-lightOrange'}`}  />
								)
							})}
						</div>
						<p className='text-sm font-medium text-gray-500'>{`(25 reviews)`}</p>
					</div>
					<PriceView price={product?.price} discount={product?.discount} label={product?.label} className='font-bold text-lg' />
					{product?.stock && (
						<p className='bg-green-100 w-24 text-center text-gree-600 text-sm py-2.5 font-semibold rounded-lg'>In Stock</p>
					)}
